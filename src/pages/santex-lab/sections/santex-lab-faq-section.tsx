...
export function SantexLabFaqSection() {
  const [openIndex, setOpenIndex] = useState<number | null>(null);

  return (
    <SantexLabSectionFrame className="border-b border-white/5 bg-[#020a09] py-16 sm:py-20">
            ...
            return (
              <div key={q} className="group px-4 py-0 sm:px-5">
                ...
                >
                  ...
                  <Plus
                    className={`size-5 shrink-0 text-teal-400 transition-transform duration-300 ${isOpen ? "rotate-45" : "rotate-0"}`}
                    aria-hidden
                  />
                </button>
                <div
                ...
                >
                  <p
                    className={`min-h-0 pb-4 text-sm leading-relaxed text-zinc-400 transition-all duration-300 ease-out ${isOpen ? "translate-y-0 opacity-100" : "-translate-y-1 opacity-0"}`}
                  >
                    {a}
                  </p>
                </div>

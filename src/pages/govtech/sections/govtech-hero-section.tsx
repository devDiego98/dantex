...
const highlights = [
  { label: "Ciudades inteligentes", icon: Building2 },
  { label: "Servicios públicos", icon: Landmark },
  { label: "Datos seguros", icon: Shield },
] as const;

export function GovtechHeroSection() {
  return (
    <section className="relative flex min-h-[100dvh] items-center justify-center overflow-hidden">
      <video
        className="absolute inset-0 h-full w-full object-cover"
        src={HERO_VIDEOS.govtech}
        autoPlay
        loop
        muted
        playsInline
      ...
      <div className="relative z-10 mx-auto flex w-full max-w-5xl flex-col items-center px-4 text-center sm:px-6 lg:px-8">
        <motion.p
          {...fadeUp}
          className="font-chakra mb-4 text-xs font-semibold uppercase tracking-[0.25em] text-cyan-200/85"
        >
          Industrias · GovTech
        </motion.p>
        ...
        >
          ...
          <br className="hidden sm:block" /> y servicios públicos
        </motion.h1>
        ...
        >
          Modernizamos operaciones y ofrecemos experiencias innovadoras,
        ...
        >
          {highlights.map((h) => (
            ...
            >
              <h.icon className="size-4 text-cyan-200/90" aria-hidden />
              {h.label}
            </span>
          ))}
        </motion.div>
